...
| IMP-02 | Improvement | Low | `dispatch/in-process.ts:12`, `container.ts:211`, `persistent.ts:30` | ✅ **DONE** — Dispatch payload construction hand-rolled 3× |
| IMP-03 | Improvement | Low | `config/load.ts:36`, `install.ts:892,1043` | ✅ **DONE** — Config/compose path-candidate lists duplicated 3× and already drifting |
| IMP-04 | Improvement | Low | `setup/env-file.ts:41`, `setup/yaml-edit.ts:13`, `setup/mcp-edit.ts` | ✅ **DONE** — Non-atomic config/secret writes (no temp-file + rename) |
| IMP-05 | Improvement | Low | multiple (`dispatch/persistent.ts:80`, `channels/telegram.ts:174`, providers) | 🟡 **PARTIAL** — `as`-casts at trust boundaries instead of validated parsing |
| IMP-06 | Improvement | Low | `brain/agents.ts:20`, `commands.ts:48`, `skills.ts:64`, `composer.ts:9` | ✅ **DONE** — `.md`-listing logic reimplemented 4× |
| IMP-07 | Improvement | Low | `channels/telegram.ts:82-98`, `whatsapp.ts:39` | ✅ **DONE** — Partial-delivery (text ok, attachment failed) never surfaced to the user |
| IMP-08 | Improvement | Low | `providers/openai.ts:33`, `anthropic.ts:29`, `index.ts:34` | ⚪ **ACCEPTED** — Placeholder API-key fallbacks mask misconfiguration until a downstream 401 |
| IMP-09 | Improvement | Low | `mcp/client.ts:97-111` | ⚪ **ACCEPTED** — MCP tool args forwarded without validating against the tool's `inputSchema` |
| IMP-10 | Improvement | Low | `scheduler/cron.ts:76-108` vs `poller.ts:107-156` | ✅ **DONE** — Static-cron path silently discards the agent's reply; poller path delivers it |
...
- **IMP-02 (Low)** — **✅ DONE (commit `316d82a`).** Extracted `originFields(args)` in `dispatch/base.ts`; the in-process, persistent, and agent-worker payloads spread it instead of hand-rolling the conditional origin spreads. (The container path uses CLI argv, a different shape, so it keeps its own.)
- **IMP-03 (Low)** — **✅ DONE (commit `0c5ce09`).** Extracted `configCandidates()` in `config/load.ts` and made `dae install`'s `ensureConfig` use it, so install now looks in exactly the same places the runtime loads from (it previously missed `.yml`/`.json` + some `~/.daedalus` variants — a real drift bug). `findComposeFile` (compose-file location, a different concern) left as-is.
- **IMP-04 (Low)** — **✅ DONE (commit `9f7a518`).** New shared `setup/atomic-write.ts` (`atomicWrite(path, data, {mode})` — temp file + rename, with `0o600`/`0o700` for secret files); `env-file.ts`, `yaml-edit.ts`, and `mcp-edit.ts` all use it now (also folds in the SEC-08 perms + SEC-14 path), so an interrupted write can't truncate a config/secret file. `mcp-edit` also no longer silently resets a non-empty-but-malformed config to `{}` — it throws so the operator fixes it rather than wiping every server.
- **IMP-05 (Low)** — **🟡 PARTIAL.** The highest-risk casts — the cross-process `DispatchResult` boundaries — are now validated: the container path is sentinel-framed + status-checked (BUG-01) and the persistent-worker response is status-validated (BUG-18). The remaining casts are low-risk and accepted: a malformed Telegram API response just triggers the poll loop's backoff (not a crash), the provider `media_type` cast surfaces as an opaque API error at worst, and the undici-dispatcher cast is a typing nicety. Adding zod schemas for the Telegram wire types is available if desired but not warranted.
- **IMP-06 (Low)** — **✅ DONE (commit `316d82a`).** Extracted `listMarkdownNames(dir)` (`brain/list-markdown.ts`), used by `listAgents` + `listCommandNames`. Re-verified the other two cited sites are NOT the same pattern: skills lists *directories* containing SKILL.md, and composer reads file *bodies* — so they correctly keep their own logic (audit over-counted).
- **IMP-07 (Low)** — **✅ DONE (commit `316d82a`).** The Telegram channel now collects failed outbound attachment sends and posts a short follow-up ("⚠️ I couldn't deliver …") so a partial delivery isn't silent. (WhatsApp is a text-only stub — no outbound attachments.)
- **IMP-08 (Low)** — **⚪ ACCEPTED (no change).** Re-verified: `resolveProviderKey` runs at agent-start and *already* throws loudly for any cloud provider missing a key, before the adapter is built. The adapter placeholders (`"no-key"`/`"ollama"`) are intentionally required for keyless local endpoints (LiteLLM/vLLM/Ollama — the SDK needs a non-empty apiKey string); removing them would break those. Not a real masking risk.
- **IMP-09 (Low)** — **⚪ ACCEPTED (no change).** The MCP **server** validates its own tool input against the advertised `inputSchema` (it's authoritative). Re-validating client-side would need a JSON-Schema validator (a new dependency — the project uses zod, not JSON Schema) for marginal defense-in-depth. Not justified; accepted.
- **IMP-10 (Low)** — **✅ DONE (commit `<pending-imp10>`).** Established this is by-design (a static schedule has no inherent delivery target — it fires with a synthetic identity), then added the feature: an optional `deliverTo: { channel, externalUserId }` field on `brain/schedules/*.yaml`. When set (and the message bus is available, i.e. under `dae serve`), cron.ts delivers the agent's reply (text + resolved attachments) to that channel/user; otherwise it's logged as before. Documented in the example schedule.
