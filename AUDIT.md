...
- **IMP-02 (Low)** — **✅ DONE (commit `<pending-impb>`).** Extracted `originFields(args)` in `dispatch/base.ts`; the in-process, persistent, and agent-worker payloads spread it instead of hand-rolling the conditional origin spreads. (The container path uses CLI argv, a different shape, so it keeps its own.)
...
- **IMP-06 (Low)** — **✅ DONE (commit `<pending-impb>`).** Extracted `listMarkdownNames(dir)` (`brain/list-markdown.ts`), used by `listAgents` + `listCommandNames`. Re-verified the other two cited sites are NOT the same pattern: skills lists *directories* containing SKILL.md, and composer reads file *bodies* — so they correctly keep their own logic (audit over-counted).
- **IMP-07 (Low)** — **✅ DONE (commit `<pending-impb>`).** The Telegram channel now collects failed outbound attachment sends and posts a short follow-up ("⚠️ I couldn't deliver …") so a partial delivery isn't silent. (WhatsApp is a text-only stub — no outbound attachments.)
