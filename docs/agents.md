...
| `execution` | No | `server` \| `executor` — `server` | Where this agent's tools run **when spawned as a sub-agent**. `executor` = the user's machine via their connected `dae remote` / desktop executor — for sub-agents whose tooling lives on the host. Spawning one without a connected executor fails fast. Top-level turns ignore this (their placement is the per-conversation toggle). |
